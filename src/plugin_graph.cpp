...
bool Graph::connect(int from, int to)
{
    if (from == to || !impl->nodes.count(from) || !impl->nodes.count(to) ||
        to == kInput || from == kOutput || impl->hasEdge(from, to))
        return false;
    Impl::Edge e;
    e.from = from;
    e.to = to;
...
bool Graph::disconnect(int from, int to)
{
    auto& es = impl->edges;
    size_t before = es.size();
    es.erase(std::remove_if(es.begin(), es.end(),
                            [&](auto& e) { return e.from == from && e.to == to; }),
             es.end());
    impl->prepared = false;
...
bool Graph::prepare(double sampleRate, uint32_t maxBlockFrames)
{
    ...
    for (auto& [id, n] : impl->nodes) {
        if (n.instance && !n.instance->prepare(sampleRate, maxBlockFrames))
            return false;
...
bool Graph::process(const float* const* in, float* const* out, uint32_t frames)
{
    ...
    for (int id : impl->order) {
        ...
        if (id == kInput) {
            ...
            ok = n.instance->process(ins, 2, outs, 2, frames) && ok;
        } else {
